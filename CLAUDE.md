...
Two follow-up gotchas after the Shopify-side variant add:
1. **New variants get stocked at the MANUAL location (2920 Peppertree Lane), not the Printful app location** — so even after Printful pushes 9999 units to its app location, `availableForSale` stays FALSE (the app location isn't in the variant's fulfillment locations). Fix: variant page → Inventory → Locations pencil ("Edit locations") → check "App Printful", uncheck the manual location → Save. Availability flips within ~1-2 min.
2. **Printify republish with prices synced RESETS Shopify prices to Printify's retail defaults** (Classic tee went $27.99 → $23.20; new size variants also arrive at Printify defaults). Re-check prices in the bulk editor (`/bulk/product_variant?...&edit=price&product_id=...`) after every republish.
